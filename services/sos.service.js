...
import { getEmergencyContactLocal } from './elderlyLocal.service'
...
export async function triggerSOS(elderlyUserId) {
  ...
  const local = await getEmergencyContactLocal(elderlyUserId)
  const digits = local.phone.replace(/\D/g, '')
  if (digits.length >= 10) {
    await Linking.openURL(`telprompt:${digits}`)
    return {
      calledNumber: digits,
      calledName: local.name?.trim() || 'Emergency contact',
      type: 'local_contact',
    }
  }
